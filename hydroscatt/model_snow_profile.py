...
def main():
    ...
    for temp in temp_vec:
        ...
        m_shell = refractive_index_melting_snow_shell(
            m_pure_ice, m_water, m_air, dens_ds_shell, dens_shell, fmw,
            d_core, d_ms)

        # internal diameter output can have arbitrary values
        d_core_out = deepcopy(d_core)

        ind = np.where(fmw == 1.)[0]
        if ind.size > 0:
            d_core_out[ind] = 0.001
            m_core[ind] = m_water
            m_shell[ind] = m_water
        ...
        fname_model_scatt = write_melting_hydro_scatt_model(
            d_ms, d_core_out, ar_ms, ar_ds, m_shell, m_core, fname_model_scatt)
        ...
        fname = write_melting_snow_part_model(
            d_ds, d_ms, d_core_out, ar_ms, ar_ds, mass_snow, fmw, vel, temp,
            fname)
