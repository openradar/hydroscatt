...
modelling of the characteristics of hail
...
def main():
    ...
    parser.add_argument(
        '--write_intermediate_results', type=int,
        default=0,
        help=(
            'Whether to write intermediate_results. Default 0 (False)'
            'Default 1 (True)'))
    ...
    for temp, alt, p_air, dens_air, visc in zip(temp_vec, alt_vec, p_air_vec,
        ...
        if args.write_intermediate_results:
            ...
            fname = (
                f'{savedir}sp_{hydro_type}_{int(temp*100):04d}'
                f'_model_part.csv')
            fname = write_melting_hail_part_model(
                diam, d_ext, d_int, axis_ratio_ext, axis_ratio_int, mass_hail,
                mass_shed, fmw, alpha, prob_break, vel, temp, fname)
            print(f'written {fname}')

    mass_shed = mass_hail_init - mass_hail

    # compute axis ratio of internal ice core and external melting part
    axis_ratio_int = compute_axis_ratio_dh(d_int)
    axis_ratio_ext = compute_axis_ratio_mh(d_ext, fmw)

    # compute the refractive indices:
    m_water = refractive_index_water(wavelength, args.temp)
    ...
    if ind.size > 0:
        ...
        m_core[ind] = m_water
        d_int[ind] = 0.001  # set to arbitrary value

    ind = np.where(fmw < 1.)[0]
    if ind.size > 0:
        # values for the core in the process of melting
        m_core[ind] = refractive_index_melting_hail_core(
            wavelength, mass_hail[ind], alpha[ind], fmw[ind], d_int[ind])
    ...
    if ind.size > 0:
        ...
        m_ice_core = refractive_index_ice_particle(m_pure_ice, dens_hail_init)
    ...
    fname_model_scatt = write_melting_hydro_scatt_model(
        d_ext, d_int, axis_ratio_ext, axis_ratio_int, m_water_coat, m_core,
        fname_model_scatt)
    ...
    fname = write_melting_hail_part_model(
        diam, d_ext, d_int, axis_ratio_ext, axis_ratio_int, mass_hail,
        mass_shed, fmw, alpha, prob_break, vel, args.temp, fname)
    print(f'written {fname}')
